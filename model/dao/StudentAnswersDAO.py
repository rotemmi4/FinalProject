...
def get_answer_by_test_name2(testName, visualization):
...
def get_answer_by_test_name(testName, visualization):
    return ("""SELECT StudentInfo.studentID,visualiztions.type,COUNT(is_correct)*1.25
     FROM StudentAnswers
     INNER JOIN StudentInfo on StudentAnswers.studentID = StudentInfo.studentID 
     INNER JOIN questions on questions.question_id = StudentAnswers.question_id 
     INNER JOIN visualiztion_properties on visualiztion_properties.text_id = questions.text_id 
...
def get_answer_by_test_name3(testName, visualization):
